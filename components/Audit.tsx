...
const Audit: React.FC<AuditProps> = ({ id, name, status, logo, date, description, tags, prizepool }) => {
  ...
  return (
    <Link href={`/contests/${id}${queryParams}`}>
      <div className='border-2 rounded-2xl bg-transparent p-10 flex justify-between max-w-4xl mx-auto'>
        <Image src={logo} alt={`${name} logo`} width={48} height={48} className='w-12 h-12 mb-2' />
        ...
        <div className='flex flex-col gap-4'>
          <p className='text-left text-lg mt-2 font-bold'>Valid until: {date}</p>
          <p className='text-left text-lg mt-2 font-bold'>Prize pool: {prizepool}</p>
        </div>
      </div>
    </Link>
  );
